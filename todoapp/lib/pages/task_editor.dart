...
class _EditTaskScreenState extends State<EditTaskScreen> {
  ...
  void _initializeData() {
    ...
    final editTaskProvider = Provider.of<EditTaskProvider>(
      context,
      listen: false,
    );
    ...
    try {
      ...
      int priorityIndex = editTaskProvider.priorities.indexOf(task.priority);
      if (priorityIndex == -1) priorityIndex = 0;

      editTaskProvider.initialize(task.id, 3, priorityIndex, task.getAlert);
    } catch (e) {
      // If task not found, pop the screen
      Navigator.pop(context);
    }
  }
  ...
  @override
  Widget build(BuildContext context) {
    final editTaskProvider = Provider.of<EditTaskProvider>(context);
    final taskProvider = Provider.of<TaskProvider>(context, listen: false);
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        centerTitle: true,
        title: Text(
          nameController.text.isEmpty ? "Edit Task" : nameController.text,
          ...
          children: [
            ...
            SizedBox(
              ...
              child: Row(
                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                children: List.generate(weekDays.length, (index) {
                  final isSelected = index == editTaskProvider.selectedDayIndex;
                  return GestureDetector(
                    onTap: () => editTaskProvider.updateSelectedDay(index),
                    child: Container(
                      width: 46,
                      decoration: isSelected
                          ? BoxDecoration(
            ...
            const SizedBox(height: 12),
            Row(
              children: List.generate(editTaskProvider.priorities.length, (
                idx,
              ) {
                final selected = idx == editTaskProvider.selectedPriority;
                return GestureDetector(
                  onTap: () => editTaskProvider.updatePriority(idx),
                  child: Container(
                    margin: EdgeInsets.only(right: 12),
                    padding: EdgeInsets.symmetric(horizontal: 18, vertical: 10),
                    decoration: BoxDecoration(
                    ...
                    child: Text(
                      editTaskProvider.priorities[idx],
                      style: TextStyle(
                        color: selected ? Colors.black : Colors.white54,
                        fontWeight: selected
                            ? FontWeight.bold
            ...
            Row(
              children: [
                ...
                Switch(
                  value: editTaskProvider.getAlert,
                  onChanged: editTaskProvider.toggleAlert,
                  activeTrackColor: Color(0xFFC59ADF),
                  activeColor: Color(0xFFDE83B0),
                ),
              ],
              ...
              children: [
                Expanded(
                  child: Container(
                    ...
                    child: MaterialButton(
                      onPressed: () =>
                          _updateTask(context, editTaskProvider, taskProvider),
                      child: Text(
                        "Edit Task",
                        style: TextStyle(
                          color: Colors.white,
                ...
                Expanded(
                  child: Container(
                    ...
                    child: MaterialButton(
                      onPressed: () => _deleteTask(context, taskProvider),
                      child: Text(
                        "Delete Task",
                        style: TextStyle(
                          color: Colors.white,
  ...
  void _updateTask(
    ...
    EditTaskProvider editTaskProvider,
    ...
    try {
      ...
      final updatedTask = currentTask.copyWith(
        ...
        priority:
            editTaskProvider.priorities[editTaskProvider.selectedPriority],
        getAlert: editTaskProvider.getAlert,
      );

      taskProvider.updateTask(widget.taskId, updatedTask);
      Navigator.pop(context);
